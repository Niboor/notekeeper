...
// One cache per build: the page registers /sw.js?v=<build id>, so a new release is a new worker with a new
// cache, and activation deletes every other cache (the previous release's files).
const SHELL = 'nk-shell-' + (new URL(self.location.href).searchParams.get('v') ?? 'dev')
