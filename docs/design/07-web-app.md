...
- **Menus and popovers:** every menu (a card's actions, a column's, the page's, the account menu, the bell) is rendered at the end of `<body>` with fixed positioning next to its button (`components/Popover.tsx`), so no scrolling column, the page-tab strip or the sticky Inbox can clip or cover it. It opens below its button, or above when there is more room there, is shifted so no edge is cut off, is as tall as the room allows and scrolls beyond that; on a phone the action menus are a bottom sheet. Arrow keys, Home and End move within a menu, Escape and a press outside close it and Tab leaves it (WEB-N4). The page tabs scroll sideways without a scrollbar (the edges fade where more are hidden); the page menu and "new page" sit outside that strip so they are always in reach (WEB-N1).
- **Controls and text areas:** inputs, selects and text areas without a style of their own share one look (theme tokens, 8 px radius; a zero-specificity base rule, so a component's own rules win), checkboxes use the accent colour, scrollbars are thin and without arrow buttons, and touch screens get larger targets (`pointer: coarse`). The note editor and the composer grow with their text up to 60 % of the window height.
...
| Edges | Near the edge of the board or a lane the view scrolls automatically; the page tabs in the top bar never do, so a tab does not slide from under the pointer while something is held over it. |
...
| Columns | A column is lifted by its title (the Inbox stays put). The column dims where it was, a title card follows the pointer, and an accent bar in the gap before or after the column under the pointer (its left or right half) shows where it will land; the tile after the last column is a drop zone for "last", and for a page with no columns. Dropped on another page's tab it goes to the end of that page; held over the tab it opens the page as a note does, and can be dropped between that page's columns. Moving to another page offers Undo. The keyboard and phones use the column menu: **Move left**, **Move right**, **Move to page** (WEB-22, `PATCH /categories/{id}`). |
| Page tabs | A tab is lifted like a column: it dims, a name chip hangs below the tab row, and an accent bar at the edge of the tab under the pointer (its left or right half) shows where it will land. Near the edge of the strip the tabs scroll (a note or column held over the tabs does not scroll them). A plain click still opens the page, and a tab dropped where it was changes nothing. The page menu has **Move left** and **Move right** (WEB-23, `PATCH /pages/{id}`). |
