...
- **`ci.yml`** runs the jobs `check` (`make check`), `e2e` (Chromium), `e2e-cross-browser` (Firefox and WebKit, both blocking), `images` (build and trivy scan of the three images) and `e2e-stack` (the built images behind nginx). It runs on **every pull request** and on **every push to master** (a new push to a pull request cancels its earlier run; runs on master are never cancelled), and it can be called by another workflow.
