...
`remind`, `snooze` and `done` run in one transaction with the ingest lock: a `!remind <when> <text>` creates its note (never grouped with a neighbour) and its reminder together, keyed by the command's message id so a replay changes nothing (the note's source reference is a synthetic one, so editing or deleting the command message in the chat afterwards is ignored instead of turning the note into the command line; change the note and its reminder in the app), and an unparseable time creates nothing. `!snooze` and `!done` resolve the replied-to message through the ids the bot reported for its deliveries, and only inside the identity's own conversation (SEC-BOT-11). A known command with invalid arguments returns an error `reply_text` with an example and creates nothing (MX-8). Commands from unlinked senders other than `link` and `help` behave like any unlinked message.
