...
A guard in front of the generated handlers sets `X-Robots-Tag`, `Cache-Control: no-store` and `Referrer-Policy: no-referrer`, discards any `Cookie` header, applies the per-address limit, takes the token from the header (missing: the same generic 404), and applies the per-link limit. Limits are in-memory token buckets per replica (`internal/ratelimit`): 120 requests a minute per address (burst 40), 300 per link (burst 100), and 200 MiB an hour of attachment volume per link (burst 100 MiB); a refusal is `429` with `Retry-After`. The limits that must hold across replicas (login, link creation) live in PostgreSQL. Attachment responses use the app's content-type rules but `Cache-Control: no-store`. Every request re-evaluates link validity as a live join (see [01](01-data-model.md) §10). Unknown, expired, revoked, disabled-owner and inactive-note links, malformed tokens and a disabled feature all return the same `404` body and do the same work first (a link lookup, then a look at the note, on a stand-in when there is no link), so neither the body nor the timing tells them apart (SEC-SHR-3). The session cookies are host-only, with no `Domain`, so browsers never send them to the share hostname. Rate limits per IP and per link apply, with a per-link request and bandwidth ceiling (CORE-SH9). The page fetches attachments with the header and displays them through blob URLs; at the 25 MiB cap this is acceptable, at the price of no seeking in large audio or video.
