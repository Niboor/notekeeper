...
def read_requirements():
    ...
    for name in ("requirements.md", "security-requirements.md"):
        ...
        for m in re.finditer(
            r"^\| ((?:CORE|AUTH|BOT|GRP|EDT|MX|WEB|AND|NFR|SEC)-[A-Za-z0-9-]+) \| (?:(Must|Should|Could|—) \| )?",
            text,
            re.M,
        ):
            found[m.group(1)] = (name, m.group(2) or "—")
    return found
...
def main():
    ...
    w("Generated by `docs/design/tools/traceability.py`; edit the mapping in that script, not this file. "
      ...
      "if a **Must** is only \"deferred\", or if the mapping names a requirement that no longer exists.")
    w("")
    ...
    def section(title, prefixes, filename):
        ...
        w(f"## {title}")
        w("")
        w("| ID | Pri | Implemented in | Verified by |")
        w("|---|---|---|---|")
        for i in ids:
            ref, tiers = MAP.get(i, ("**MISSING**", "-"))
            w(f"| {i} | {reqs[i][1]} | {ref} | {tiers} |")
        w("")
