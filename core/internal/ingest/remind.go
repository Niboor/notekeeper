...
func (s *Service) remind(ctx context.Context, bot *bots.Principal, ident bots.Identity, cmd Command) (CommandOutcome, error) {
	...
	err := s.St.InUserTx(ctx, ident.UserID, func(tx *store.UserTx) error {
		...
		if cmd.MessageID != "" { // replay: the command was already handled
			id := "remind:" + cmd.MessageID
		...
		if cmd.MessageID != "" {
			...
			return tx.Q.UpdateIngestResult(ctx, dbq.UpdateIngestResultParams{BotInstanceID: bot.InstanceID, EventID: "remind:" + cmd.MessageID, Result: raw, UserID: tx.UserID})
		}
		return nil
	})
