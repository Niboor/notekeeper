...
func serve(ctx context.Context) error {
	...
	workers, err := jobs.New(pool, jobs.Deps{Store: st, Blobs: sv.Blobs, Log: log})
	if err != nil {
		return err
	}
	...
	routers, err := server.NewRouters(server.Deps{
		...
		Accounts: sv.Accounts, Bots: sv.Bots, Notes: sv.Notes, Board: sv.Board, Blobs: sv.Blobs, Ingest: sv.Ingest, Hub: hub,
